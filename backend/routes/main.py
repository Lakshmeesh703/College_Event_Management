...
from ..models import Event, EventParticipation, Participant, Result, Team, User, db
...
def _legacy_redirect_coordinator():
    ...
    if session.get("user_role") in (ROLE_COORDINATOR, ROLE_CONVENER, ROLE_ADMIN, ROLE_MANAGEMENT):
...
def lookup_results():
    ...
    if event_id:
        ...
        if chosen:
            rows = (
                db.session.query(Result, Participant, Team)
                .outerjoin(Participant, Result.participant_id == Participant.id)
                .outerjoin(Team, Result.team_id == Team.id)
                .filter(Result.event_id == event_id)
                .order_by(Result.rank.asc().nulls_last(), Team.name.asc().nulls_last(), Participant.name.asc())
                .all()
            )
