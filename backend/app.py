...
def _ensure_schema_updates():
    ...
    if not _column_exists("event_participation", "competition_id"):
        ...
        statements.append(
            "ALTER TABLE event_participation ADD CONSTRAINT fk_event_participation_competition_id "
            "FOREIGN KEY (competition_id) REFERENCES competitions(id) ON DELETE CASCADE"
        )
    if not _column_exists("event_participation", "team_id"):
        statements.append("ALTER TABLE event_participation ADD COLUMN team_id INTEGER")
        statements.append(
            "ALTER TABLE event_participation ADD CONSTRAINT fk_event_participation_team_id "
            "FOREIGN KEY (team_id) REFERENCES teams(id) ON DELETE CASCADE"
        )

    if not _column_exists("results", "competition_id"):
        statements.append("ALTER TABLE results ADD COLUMN competition_id INTEGER")
        statements.append(
            "ALTER TABLE results ADD CONSTRAINT fk_results_competition_id "
            "FOREIGN KEY (competition_id) REFERENCES competitions(id) ON DELETE CASCADE"
        )
    if not _column_exists("results", "team_id"):
        statements.append("ALTER TABLE results ADD COLUMN team_id INTEGER")
        statements.append(
            "ALTER TABLE results ADD CONSTRAINT fk_results_team_id "
            "FOREIGN KEY (team_id) REFERENCES teams(id) ON DELETE CASCADE"
        )
    ...
    if not _column_exists("coordinator_profiles", "allotted_competition_id"):
        statements.append("ALTER TABLE coordinator_profiles ADD COLUMN allotted_competition_id INTEGER")

    if dialect == "postgresql":
        statements.append("CREATE INDEX IF NOT EXISTS ix_event_participation_competition_id ON event_participation (competition_id)")
        statements.append("CREATE INDEX IF NOT EXISTS ix_event_participation_team_id ON event_participation (team_id)")
        statements.append("CREATE INDEX IF NOT EXISTS ix_results_competition_id ON results (competition_id)")
        statements.append("CREATE INDEX IF NOT EXISTS ix_results_team_id ON results (team_id)")
        statements.append("ALTER TABLE event_participation DROP CONSTRAINT IF EXISTS uq_event_participant")
        statements.append("ALTER TABLE results DROP CONSTRAINT IF EXISTS uq_result_event_participant")
        statements.append(
            "DO $$ BEGIN "
            "IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uq_event_participant_competition') THEN "
            "ALTER TABLE event_participation ADD CONSTRAINT uq_event_participant_competition UNIQUE (event_id, participant_id, competition_id); "
            "END IF; "
            "END $$;"
        )
        statements.append(
            "DO $$ BEGIN "
            "IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uq_result_event_participant_competition') THEN "
            "ALTER TABLE results ADD CONSTRAINT uq_result_event_participant_competition UNIQUE (event_id, participant_id, competition_id); "
            "END IF; "
            "END $$;"
        )
...
def _ensure_default_admin_account():
    """Guarantee one known admin login for administration tasks."""
