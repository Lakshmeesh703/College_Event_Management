...
Relationships:
  ...
  Event 1—* EventParticipation *—0..1 Team
  Event 1—* Result *—1 Participant
  Event 1—* Result *—0..1 Team
...
class Event(db.Model):
    ...
    teams = db.relationship(
        "Team", back_populates="event", cascade="all, delete-orphan"
    )
...
class Participant(db.Model):
    ...
    university_mail = db.Column(db.String(255), nullable=True, unique=True, index=True)
...
class Competition(db.Model):
    ...
    __table_args__ = (
        db.Index("ix_competitions_event_id", "event_id"),
        db.Index("ix_competitions_date", "date"),
    )
    ...
    max_participants = db.Column(db.Integer, nullable=True)
    is_team_event = db.Column(db.Boolean, default=False, nullable=False)
    ...
    participations = db.relationship(
        "EventParticipation", back_populates="competition", cascade="all, delete-orphan"
    )
    results = db.relationship(
        "Result", back_populates="competition", cascade="all, delete-orphan"
    )

    def __repr__(self):
        return f"<Competition {self.id} event={self.event_id} {self.name!r}>"
...
class Team(db.Model):
    ...
    __table_args__ = (
        ...
        db.Index("ix_teams_event_id", "event_id"),
        db.Index("ix_teams_competition_id", "competition_id"),
        db.Index("ix_teams_captain_participant_id", "captain_participant_id"),
    )

    id = db.Column(db.Integer, primary_key=True)
    event_id = db.Column(db.Integer, db.ForeignKey("events.id", ondelete="CASCADE"), nullable=False)
    competition_id = db.Column(db.Integer, db.ForeignKey("competitions.id", ondelete="CASCADE"), nullable=False)
    captain_participant_id = db.Column(db.Integer, db.ForeignKey("participants.id"), nullable=False)
    name = db.Column(db.String(160), nullable=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)

    event = db.relationship("Event", back_populates="teams")
    competition = db.relationship("Competition", back_populates="teams")
    captain = db.relationship("Participant")
    members = db.relationship("TeamMember", back_populates="team", cascade="all, delete-orphan")
    participations = db.relationship("EventParticipation", back_populates="team", cascade="all, delete-orphan")
    results = db.relationship("Result", back_populates="team", cascade="all, delete-orphan")

    def __repr__(self):
        return f"<Team {self.id} competition={self.competition_id} {self.name!r}>"
...
class TeamMember(db.Model):
    ...
    __table_args__ = (
        ...
        db.Index("ix_team_members_participant_id", "participant_id"),
    )

    id = db.Column(db.Integer, primary_key=True)
    team_id = db.Column(db.Integer, db.ForeignKey("teams.id", ondelete="CASCADE"), nullable=False)
    participant_id = db.Column(db.Integer, db.ForeignKey("participants.id"), nullable=True)
    member_order = db.Column(db.Integer, nullable=False, default=1)
    ...
    def __repr__(self):
        return f"<TeamMember {self.id} team={self.team_id} {self.name!r}>"
...
class CoordinatorProfile(db.Model):
    ...
    school = db.Column(db.String(160), nullable=True, index=True)
    allotted_event_id = db.Column(db.Integer, db.ForeignKey("events.id"), nullable=True, index=True)
    allotted_competition_id = db.Column(db.Integer, db.ForeignKey("competitions.id"), nullable=True, index=True)
...
class EventParticipation(db.Model):
    ...
    __table_args__ = (
        UniqueConstraint("event_id", "participant_id", "competition_id", name="uq_event_participant_competition"),
        db.Index("ix_event_participation_event_id", "event_id"),
        db.Index("ix_event_participation_participant_id", "participant_id"),
        db.Index("ix_event_participation_competition_id", "competition_id"),
        db.Index("ix_event_participation_team_id", "team_id"),
    )
    ...
    competition_id = db.Column(db.Integer, db.ForeignKey("competitions.id", ondelete="CASCADE"), nullable=True)
...
class Result(db.Model):
    ...
    __table_args__ = (
        UniqueConstraint("event_id", "participant_id", "competition_id", name="uq_result_event_participant_competition"),
        db.Index("ix_results_event_id", "event_id"),
        db.Index("ix_results_participant_id", "participant_id"),
        db.Index("ix_results_competition_id", "competition_id"),
        db.Index("ix_results_team_id", "team_id"),
